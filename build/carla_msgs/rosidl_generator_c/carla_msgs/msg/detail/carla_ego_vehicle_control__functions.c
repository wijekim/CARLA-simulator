...
carla_msgs__msg__CarlaEgoVehicleControl__create(void)
{
  ...
  carla_msgs__msg__CarlaEgoVehicleControl__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    carla_msgs__msg__CarlaEgoVehicleControl * data =
      (carla_msgs__msg__CarlaEgoVehicleControl *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!carla_msgs__msg__CarlaEgoVehicleControl__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          carla_msgs__msg__CarlaEgoVehicleControl__fini(&output->data[i]);
        }
        return false;
      }
    }
