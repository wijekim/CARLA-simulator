  ...
  carla_msgs__msg__CarlaLaneInvasionEvent__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    carla_msgs__msg__CarlaLaneInvasionEvent * data =
      (carla_msgs__msg__CarlaLaneInvasionEvent *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!carla_msgs__msg__CarlaLaneInvasionEvent__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          carla_msgs__msg__CarlaLaneInvasionEvent__fini(&output->data[i]);
        }
        return false;
      }
    }
