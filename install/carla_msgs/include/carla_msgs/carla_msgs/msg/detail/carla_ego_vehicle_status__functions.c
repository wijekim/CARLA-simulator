  ...
  carla_msgs__msg__CarlaEgoVehicleStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    carla_msgs__msg__CarlaEgoVehicleStatus * data =
      (carla_msgs__msg__CarlaEgoVehicleStatus *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!carla_msgs__msg__CarlaEgoVehicleStatus__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          carla_msgs__msg__CarlaEgoVehicleStatus__fini(&output->data[i]);
        }
        return false;
      }
    }
